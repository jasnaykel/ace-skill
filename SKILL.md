...
## Correcciones obligatorias de scaffolding ACE

### BROKER SCHEMA y referencias ESQL

El `BROKER SCHEMA` se deriva de la ruta lÃ³gica ESQL separando cada segmento con punto. Para `ace/esb/payexe/pro/hub/init/s`, el valor obligatorio es:

```esql
BROKER SCHEMA ace.esb.payexe.pro.hub.init.s
```
...
La carpeta `Referenced Libraries` del ACE Toolkit es un contenedor lÃ³gico; no se crea como carpeta fÃ­sica vacÃ­a. Para que aparezca, la aplicaciÃ³n debe incluir en `APP_<S>/.project`, dentro de `<projects>`, los proyectos `LIB_CORE_CONTROL`, `LIB_CORE_COMMON` y `LIB_SMF_UTIL`, y en `application.descriptor`, dentro de `<references>`, sus tres elementos `<sharedLibraryReference>` correspondientes.

### Policy projects por ambiente

Cada ruta `src/v1.0/configuration/{DEV,QAS,PRD}/policyproject/PLP_<S>/` debe ser un proyecto ACE Policy completo y contener `.project`, `.settings/org.eclipse.core.resources.prefs` y `policy.descriptor`, ademÃ¡s de `PL_UserDefined`, `PL_ActiveDirectory` y `Monitoring`. El `.project` debe usar la nature `com.ibm.etools.mft.policy.ui.Nature` y el builder `com.ibm.etools.mft.policy.ui.policybuilder`.
